...
describe Cat do
  ...
  subject(:frodo) { described_class.new("Frodo", nil, "orange") }
  # Write a test using the second shared_example to test that cat responds to
  # talk ('meow').
  context 'when the method name is the same in multiple classes' do
    include_examples "shared method name"
  end

  # remove the 'x' before running this test
  it 'is not hungry' do
    expect(frodo).not_to be_hungry
  end

  # remove the 'x' before running this test
  it 'is hiding' do
    expect(frodo).to be_hiding
  end
end
