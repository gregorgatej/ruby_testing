...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:game_continues) { described_class.new(8, '1') }
      # remove the 'x' before running this test
  ...
  describe '#player_turn' do
    ...
    context 'when user input is valid' do
      ...
      it 'stops loop and does not display error message (using spies)' do
        valid_input = '4'
        allow(game_loop).to receive(:player_input).and_return(valid_input)
        spy = allow(game_loop).to receive(:puts)
        game_loop.player_turn
        expect(game_loop).not_to have_received(:puts).with('Input error!')
      end
    end
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        first_invalid_input = 'm'
        second_invalid_input = '99'
        valid_input = '4'
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      # remove the 'x' before running this test
      subject(:game_two) { described_class.new(6, '6', 3) }
    ...
    context 'when count is 4 and over' do
      subject(:game_four) { described_class.new(1, '1', 4) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        hard_recognition_phrase = "That was hard. It took you 4 guesses!\n"
        expect { game_four.final_message }.to output(hard_recognition_phrase).to_stdout
      end
    end
  end
end
